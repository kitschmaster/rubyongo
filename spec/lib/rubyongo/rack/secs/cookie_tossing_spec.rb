...
describe Rubyongo::Rack::Secs::CookieTossing do
...
END
  ...
  context 'with redirect reaction' do
    before(:each) do
      mock_app do
        use Rubyongo::Rack::Secs::CookieTossing, :reaction => :redirect
        run DummyApp
      end
    end
  ...
  context 'with custom session key' do
    it 'denies requests with duplicate session cookies' do
      mock_app do
        use Rubyongo::Rack::Secs::CookieTossing, :session_key => '_session'
        run DummyApp
      end
