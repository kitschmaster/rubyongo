...
describe Rubyongo::Rack::Secs::AuthenticityToken do
  ...
  it "allows for a custom authenticity token param" do
    mock_app do
      use Rubyongo::Rack::Secs::AuthenticityToken, :authenticity_param => 'csrf_param'
  ...
  describe ".token" do
    it "returns a unique masked version of the authenticity token" do
      expect(Rubyongo::Rack::Secs::AuthenticityToken.token(session)).not_to eq(masked_token)
    end

    it "sets a session authenticity token if one does not exist" do
      session = {}
      allow(Rubyongo::Rack::Secs::AuthenticityToken).to receive(:random_token).and_return(token)
      allow_any_instance_of(Rubyongo::Rack::Secs::AuthenticityToken).to receive(:mask_token).and_return(masked_token)
      Rubyongo::Rack::Secs::AuthenticityToken.token(session)
