...
describe Rubyongo::Rack::Secs do
  it_behaves_like "any rack application"

  it 'passes on options' do
    mock_app do
      use Rubyongo::Rack::Secs, :track => ['HTTP_FOO']
  ...
  it 'passes errors through if :reaction => :report is used' do
    mock_app do
      use Rubyongo::Rack::Secs, :reaction => :report
  ...
  describe "#react" do
    it 'prevents attacks and warns about it' do
      io = StringIO.new
      mock_app do
        use Rubyongo::Rack::Secs, :logger => Logger.new(io)
        run DummyApp
      end
    ...
    it 'reports attacks if reaction is to report' do
      io = StringIO.new
      mock_app do
        use Rubyongo::Rack::Secs, :reaction => :report, :logger => Logger.new(io)
        run DummyApp
      end
      post('/', {}, 'rack.session' => {}, 'HTTP_ORIGIN' => 'http://malicious.com')
    ...
    it 'passes errors to reaction method if specified' do
      io = StringIO.new
      Rubyongo::Rack::Secs::Base.send(:define_method, :special) { |*args| io << args.inspect }
      mock_app do
        use Rubyongo::Rack::Secs, :reaction => :special, :logger => Logger.new(io)
        run DummyApp
      end
      post('/', {}, 'rack.session' => {}, 'HTTP_ORIGIN' => 'http://malicious.com')
  ...
  describe "#html?" do
    context "given an appropriate content-type header" do
      subject { Rubyongo::Rack::Secs::Base.new(nil).html? 'content-type' => "text/html" }
      it { is_expected.to be_truthy }
    end

    context "given an appropriate content-type header of text/xml" do
      subject { Rubyongo::Rack::Secs::Base.new(nil).html? 'content-type' => "text/xml" }
      it { is_expected.to be_truthy }
    end

    context "given an appropriate content-type header of application/xml" do
      subject { Rubyongo::Rack::Secs::Base.new(nil).html? 'content-type' => "application/xml" }
      it { is_expected.to be_truthy }
    end

    context "given an inappropriate content-type header" do
      subject { Rubyongo::Rack::Secs::Base.new(nil).html? 'content-type' => "image/gif" }
      it { is_expected.to be_falsey }
    end

    context "given no content-type header" do
      subject { Rubyongo::Rack::Secs::Base.new(nil).html?({}) }
      it { is_expected.to be_falsey }
    end
  end
  ...
  describe "new" do
    it 'should allow disable session protection' do
      mock_app do
        use Rubyongo::Rack::Secs, :without_session => true
        run DummyApp
      end
