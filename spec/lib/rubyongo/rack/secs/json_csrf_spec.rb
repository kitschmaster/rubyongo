...
describe Rubyongo::Rack::Secs::JsonCsrf do
  ...
  describe 'json response' do
    ...
    it "accepts requests with json responses with a remote referrer when allow_if is true" do
      mock_app do
        use Rubyongo::Rack::Secs::JsonCsrf, :allow_if => lambda{|env| env['HTTP_REFERER'] == 'http://good.com'}
  ...
  describe 'with drop_session as default reaction' do
    it 'still denies' do
      mock_app do
        use Rubyongo::Rack::Secs, :reaction => :drop_session
        run proc { |e| [200, {'Content-Type' => 'application/json'}, []]}
      end
