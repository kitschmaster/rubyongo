...
function uploadFiles(files) {
  ...
  if (Tests.formdata) {
    ...
    formData.append('path', $('input#upload_path').val());
  ...
  var contextmenu_items_callback = function(node) {
    ...
    tmp.upload = {
          ...
          "action"      : function (data) {
            ...
            $("input#upload_path").val(obj.id);
            //trigger file selection on the hidden file input element
            $("input#upload").trigger("click");

            console.log("Uploading from " + obj.id);
            // trigger file selection (add file input in content.erb)
            // upload -> get back ID
            // refresh tree / add new node with ID
            //inst.create_node(obj, {}, "last", function (new_node) {
            //  setTimeout(function () { inst.edit(new_node); },0);
            //});

            // use the uploaded file in the editor
            /*
